...
确认 AgentScout 项目目录存在，使用 uv 管理环境：
...
如果 `.venv` 目录不存在，先运行 `uv sync` 安装依赖。

### 第二步：运行流水线

```bash
cd /Users/bowenshu/Desktop/02_代码项目/应用与实验/playground/AgentScout && uv run python -m src.pipeline
```
